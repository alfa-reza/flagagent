...
CHAT_LARGE_WITH_TOOL = json.dumps(
    {
        "id": "chatcmpl-1",
...
def test_large_success_within_deadline(tmp_path):
    class H(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            self.send_header("Content-Length", str(len(CHAT_LARGE_NO_TOOL)))
            self.end_headers()
            self.wfile.write(CHAT_LARGE_NO_TOOL)
    ...
    assert len(mrs) == 1
    assert mrs[0]["payload"]["content"] == LARGE_CONTENT
    assert mrs[0]["payload"]["tool_calls"] == []
...
def test_large_pre_deadline_preserved(tmp_path):
    class H(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            self.send_header("Content-Length", str(len(CHAT_LARGE_WITH_TOOL)))
            self.end_headers()
            self.wfile.write(CHAT_LARGE_WITH_TOOL)
...
def test_large_incomplete_not_preserved(tmp_path):
    ...
    class _Wrapper:
        ...
        def recv(self):
            m = self._real.recv()
            if isinstance(m, dict) and "committed_at" in m:
                self.observed = m
                self.ev.set()
            return m
    ...
    def cap(*a, **kw):
        ...
        if pp is not None and "pp" not in holder:
            try:
                ...
                w = _Wrapper(real_cr)
                pp._commit_rx = w  # type: ignore[attr-defined]
                holder["wrapper"] = w
            except Exception:
                pass
            holder["pp"] = pp
